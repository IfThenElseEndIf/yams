...
def typeof(v, /):
      if isinstance(v, (int, float)):
            ...
            return "null"
      elif getattr(type(v), "_is_yams_external", False):
            return "external"
...
class External:
      ...
      def __call__(self, *args):
            if len(args) != self.args:
                  raise TypeError(f"external call expected {self.args} but got {len(args)}")
            ...
            if isinstance(result, str):
                  ...
                  raise ValueError("external call returned invalid data")
...
def external(path):
      ...
      try:
            ...
            sys.dont_write_bytecode = c
            raise Exception(f"couldn't import external file {str(path)!r}")

      sys.dont_write_bytecode = c

      return {
            name:vars(module)[name] for name in vars(module)
            if getattr(type(vars(module)[name]), "_is_yams_external", False)
      }
...
def tostr(x, /):
      if typeof(x) == "number":
            ...
            raise TypeError(f"cannot convert {typeof(x)} object to string object (list of positive integers valid as characters)")
...
def run(code, *argv, mute=False, stack=None, variables=None, macros=None, raiseonexit=False):
      ...
      SPECIAL = {
            ...
            "do", ",", "'", "print", "println", "length", "exit", "goto", "delete", "boolean", "type", "isstr", "import", "wait", "tostr", "tonum", "isnum", "not", "external", # 1
      ...
      while program_buffer:
            ...
            if isinstance(instruction, tuple):
                  ...
                  if word == "/*":
                  ...
                  elif word in {"do", ",", "'", "print", "println", "length", "exit", "goto", "delete", "boolean", "type", "isstr", "import", "wait", "tostr", "tonum", "isnum", "not", "external"}:
                        if not stack[-1]:
                              err(f"'{word} expected 1 object but got 0"); return 1
                        x = stack[-1].pop()
                        if word == "do":
                              if typeof(x) not in {"words", "function", "external"}:
                                    err(f"'{word} expected 1 words or function object but got a {typeof(x)} object"); return 1
                              if typeof(x) == "words":
                                    program_buffer[0:1] = x
                                    continue
                              elif typeof(x) == "external":
                                    if len(stack[-1]) < len(x):
                                          err(f"not enough arguments for external call (expected {len(x)})"); return 1
                                    stack[-1].append(x(*[stack[-1].pop() for _ in range(len(x))]))
                              else:
                                    if len(stack[-1]) < len(x):
                        ...
                        if word == ",":
                              ...
                              if not isstr(x):
                                    err(f"'{word} expected a string object (list of integers valid as characters) but got a {typeof(x) if typeof(x) != 'list' else 'non-string list'} object"); return 1
                              try:
                                    v = external("".join(chr(c) for c in x))
                              except Exception as e:
                                    err(str(e)); return 1
                              for k in v:
                                    if k in macros:
...
def typeof(v, /):
      if isinstance(v, (int, float)):
            ...
            return "null"
      elif isinstance(v, External) or ((type(v).__name__ == "External") and type(v).__module__ == "yams"):
            return "external"
...
def external(path):
      ...
      return {
            name:vars(module)[name] for name in vars(module)
            if type(vars(module)[name]).__name__ == "External" and type(vars(module)[name]).__module__ == "yams"
      }
